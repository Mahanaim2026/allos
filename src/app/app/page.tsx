...
return (<blockquote key={i} style={{ borderLeft: '3px solid #B8832A', paddingLeft: '1rem', margin: '1.2em 0', fontFamily: "'Spectral', Georgia, serif", fontStyle: 'italic', color: '#1A3F5C', lineHeight: 1.7 }}>{para}</blockquote>);
}
if (lines.length > 2 && lines.every(l => l.length < 80)) {
return (<div key={i} style={{ marginBottom: '1.4em', fontStyle: 'italic', color: '#1A3F5C' }}>{lines.map((l, j) => <div key={j} style={{ lineHeight: 1.6 }}>{l}</div>)}</div>);
}
return <p key={i} style={{ margin: '0 0 1.2em', color: '#0F2B45' }}>{para}</p>;
})}
</div>
);
}
...
return <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#0F2B45', opacity: 0.5, transform: `scale(${v ? 1 : 0})`, transition: 'transform 0.33s ease' }} />;
}
...
<div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#0F2B45', borderRadius: '50%', width: 48, height: 48, marginBottom: 28 }}>
<AllosLogo size={32} variant="dark" />
</div>
<p style={{ fontFamily: "'Spectral', Georgia, serif", fontStyle: 'italic', fontSize: '1.2rem', color: '#0F2B45', lineHeight: 1.65, transition: 'opacity 0.5s ease', opacity: visible ? 1 : 0 }}>&ldquo;{msg.text}&rdquo;</p>
<p style={{ fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.82rem', color: '#B8832A', letterSpacing: '0.04em' }}>&mdash; {msg.ref}</p>
...
return <div style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase' as const, color: '#4A7299', marginBottom: 8 }}>{s}</div>;
}
...
{userName && <span className="allos-nav-greeting" style={{ color: '#3D5166', fontSize: '0.9rem' }}>Welcome back, {userName}</span>}
<Link href="/journey" style={{ color: '#0F2B45', fontSize: '0.9rem', textDecoration: 'none', fontWeight: 500 }}>
Journey{journeyCount !== null ? ` (${journeyCount})` : ''}
</Link>
{userEmail ? (
<button onClick={async () => { await supabase.auth.signOut(); window.location.href = '/'; }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3D5166', fontSize: '0.9rem' }}>Sign out</button>
) : (
<Link href="/auth/login" style={{ color: '#0F2B45', fontSize: '0.9rem', textDecoration: 'none' }}>Sign in</Link>
)}
</div>
</nav>
);

const customInputStyle = { marginTop: 8, padding: '10px 14px', borderRadius: 12, border: '1.5px solid #C8D8E8', background: '#FFFFFF', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.9rem', color: '#0F2B45', width: '100%', outline: 'none', boxSizing: 'border-box' as const };

const step2 = (
<div className="allos-container" style={{ margin: '0 auto', padding: '36px 24px' }}>
<div style={{ background: '#F0F5FA', borderRadius: 12, padding: '12px 16px', marginBottom: 24, display: 'flex', flexWrap: 'wrap' as const, gap: 8 }}>
<span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#4A7299', letterSpacing: '0.1em' }}>YOUR SEASON</span>
{[mood || customMood, struggle || customStruggle, life || customLife, spirit || customSpirit].filter(Boolean).map(v => (
<span key={v} style={{ fontSize: '0.82rem', background: '#0F2B45', color: '#FFFFFF', borderRadius: 100, padding: '2px 10px' }}>{v}</span>
))}
</div>
...
<button onClick={generate} style={{ width: '100%', padding: '14px', background: '#0F2B45', color: '#FFFFFF', border: 'none', borderRadius: 100, fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '1rem', fontWeight: 600, cursor: 'pointer', letterSpacing: '0.02em' }}>Receive the Word</button>
</div>
);
...
<span key={v} style={{ fontSize: '0.75rem', fontWeight: 600, background: v === (mood || customMood) ? '#0F2B45' : '#F0F5FA', color: v === (mood || customMood) ? '#FFFFFF' : '#3D5166', borderRadius: 100, padding: '4px 12px', fontFamily: 'Hanken Grotesk, sans-serif', letterSpacing: '0.04em' }}>{v}</span>
))}
</div>
<div className="allos-result-card" style={{ background: '#FFFFFF', borderRadius: 20, padding: '28px 24px', boxShadow: '0 2px 16px rgba(15,43,69,0.08)', marginBottom: 24 }}>
<h1 style={{ fontFamily: "'Spectral', Georgia, serif", fontWeight: 400, fontSize: 'clamp(22px,5vw,30px)', color: '#0F2B45', marginBottom: 24, lineHeight: 1.3 }}>
{[mood || customMood, struggle || customStruggle].filter(Boolean).join(' & ') || 'Your word'}
</h1>
<div style={{ fontFamily: "'Spectral', Georgia, serif", fontSize: '1.05rem', color: '#0F2B45', lineHeight: 1.8 }}>{renderOutput(result)}</div>
</div>
<div style={{ marginBottom: 20 }}>
<input type="text" placeholder="\u201cWhat word or phrase from this speaks most to you right now?\u201d" style={{ width: '100%', padding: '14px 18px', borderRadius: 14, border: '1.5px solid #C8D8E8', background: '#FFFFFF', fontFamily: "'Spectral', Georgia, serif", fontStyle: 'italic', fontSize: '0.95rem', color: '#3D5166', outline: 'none', boxSizing: 'border-box' as const }} />
</div>
<div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
<button onClick={() => { setStep(1); setMood(''); setStruggle(''); setLife(''); setSpirit(''); setCustomMood(''); setCustomStruggle(''); setCustomLife(''); setCustomSpirit(''); setResult(''); setSaved(false); }} style={{ flex: 1, padding: '12px', borderRadius: 100, border: '1.5px solid #C8D8E8', background: '#FFFFFF', color: '#0F2B45', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.9rem', cursor: 'pointer' }}>New season</button>
<button onClick={saveToJourney} disabled={saved} style={{ flex: 1, padding: '12px', borderRadius: 100, background: saved ? '#3D5166' : '#0F2B45', color: '#FFFFFF', border: 'none', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.9rem', fontWeight: 600, cursor: saved ? 'default' : 'pointer' }}>{saved ? 'Saved \u2713' : 'Save to Journey'}</button>
</div>
<div style={{ marginBottom: 8 }}>
<div style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase' as const, color: '#4A7299', marginBottom: 10 }}>SHARE</div>
<div className="allos-share-row" style={{ display: 'flex', gap: 10, flexWrap: 'wrap' as const }}>
<button onClick={copyText} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 16px', borderRadius: 100, border: '1.5px solid #C8D8E8', background: '#FFFFFF', color: '#0F2B45', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.85rem', cursor: 'pointer', minHeight: 44 }}>
...
<button onClick={shareX} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 16px', borderRadius: 100, border: '1.5px solid #C8D8E8', background: '#FFFFFF', color: '#0F2B45', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.85rem', cursor: 'pointer', minHeight: 44 }}>
...
<button onClick={shareEmail} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 16px', borderRadius: 100, border: '1.5px solid #C8D8E8', background: '#FFFFFF', color: '#0F2B45', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '0.85rem', cursor: 'pointer', minHeight: 44 }}>
...
<div style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase' as const, color: '#4A7299', marginBottom: 8 }}>YOUR SEASON</div>
<h1 style={{ fontFamily: "'Spectral', Georgia, serif", fontWeight: 300, fontSize: 'clamp(22px,5vw,30px)', color: '#0F2B45', marginTop: 0, marginBottom: 8 }}>What are you carrying right now?</h1>
<p style={{ color: '#3D5166', fontSize: '0.9rem', marginTop: 0 }}>Select what feels most true. Nothing is required.</p>
</div>
<div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 6, marginBottom: 20 }}>
{[mood || customMood, struggle || customStruggle, life || customLife, spirit || customSpirit].filter(Boolean).map(v => (
<span key={v} style={{ fontSize: '0.75rem', fontWeight: 600, background: '#0F2B45', color: '#FFFFFF', borderRadius: 100, padding: '3px 10px', fontFamily: 'Hanken Grotesk, sans-serif' }}>{v}</span>
))}
</div>
...
<button onClick={() => setStep(2)} style={{ width: '100%', padding: '14px', background: '#0F2B45', color: '#FFFFFF', border: 'none', borderRadius: 100, fontFamily: 'Hanken Grotesk, sans-serif', fontSize: '1rem', fontWeight: 600, cursor: 'pointer', letterSpacing: '0.02em' }}>Continue</button>
</>
)}
{step === 2 && step2}
</div>
