...
const DENOMINATIONS = [
  'Non-denominational', 'Baptist', 'Pentecostal / Charismatic', 'Methodist', 'Anglican / Episcopal',
  'Presbyterian', 'Lutheran', 'Catholic', 'Orthodox', 'Reformed', 'Adventist', 'Other / Prefer not to say',
];
...
export default function SignupPage() {
  ...
  async function handleCreateAccount(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.auth.signUp({
      email, password,
      options: {
        data: { full_name: displayName, first_name: firstName, last_name: lastName },
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6 py-10">
      <div className="w-full max-w-sm">
        ...
        {step === 'account' && (
          <>
            ...
            <form onSubmit={handleAccountStep} className="space-y-3">
              ...
              <button type="submit"
                className="w-full bg-allos-navy text-white py-3.5 rounded-2xl font-medium text-sm hover:bg-allos-blue transition-colors">
                Continue →
              </button>
            </form>
          </>
        ...
        {step === 'profile' && (
          <>
            <h1 className="text-2xl font-serif font-medium text-allos-navy text-center mb-1">Tell us about yourself</h1>
            <p className="text-sm text-slate-400 text-center mb-8">This helps personalise your journey (all optional)</p>
            <form onSubmit={handleCreateAccount} className="space-y-3">
              <div className="flex gap-2">
                <input type="text" value={firstName} onChange={e => setFirstName(e.target.value)}
                  placeholder="First name"
                  className="flex-1 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-allos-blue" />
                <input type="text" value={lastName} onChange={e => setLastName(e.target.value)}
                  placeholder="Last name"
                  className="flex-1 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-allos-blue" />
              </div>
              <select value={denomination} onChange={e => setDenomination(e.target.value)}
                className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-600 focus:outline-none focus:border-allos-blue bg-white">
              ...
              <button type="button" onClick={() => handleCreateAccount({ preventDefault: () => {} } as React.FormEvent)}
        ...
        <p className="text-center text-sm text-slate-400 mt-6">
          ...
          <Link href="/auth/login" className="text-allos-blue font-medium hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
