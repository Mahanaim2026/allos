...
export default function SignupPage() {
  ...
  const inputStyle: React.CSSProperties = { padding: '13px 16px', border: '1.5px solid #C8D8E8', borderRadius: 12, fontSize: '0.9rem', outline: 'none', color: '#0F2B45', background: '#FFFFFF', width: '100%', boxSizing: 'border-box' };
    ...
    <div style={{ minHeight: '100vh', background: '#F0F5FA', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 440 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: '#3D5166', fontSize: '0.9rem', margin: 0 }}>
          ...
          {step === 1 && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                ...
                <span style={{ color: '#5C7A94', fontSize: '0.8rem' }}>or use email</span>
                <div style={{ flex: 1, height: 1, background: '#C8D8E8' }}/>
              </div>
              <form onSubmit={handleStep1} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <button type="submit" style={{ padding: '13px', background: '#0F2B45', color: '#FFFFFF', border: 'none', borderRadius: '100px', fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer', minHeight: 48 }}>Continue</button>
              </form>
            </>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleStep2} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <button type="submit" disabled={loading}
                style={{ padding: '13px', background: '#0F2B45', color: '#FFFFFF', border: 'none', borderRadius: '100px', fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer', minHeight: 48 }}>
                {loading ? 'Creating account...' : 'Create my account'}
              </button>
              <button type="button" onClick={() => setStep(1)} style={{ background: 'transparent', border: 'none', color: '#3D5166', cursor: 'pointer', fontSize: '0.85rem', fontFamily: 'inherit' }}>Back</button>
            </form>
          )}

          {message && <p style={{ marginTop: 14, fontSize: '0.85rem', color: '#B8832A', textAlign: 'center', lineHeight: 1.5 }}>{message}</p>}
        </div>

        <p style={{ textAlign: 'center', marginTop: 20, color: '#3D5166', fontSize: '0.85rem' }}>
          Already have an account?{' '}
          <Link href="/auth/login" style={{ color: '#2B6CB0', fontWeight: 600, textDecoration: 'none' }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
